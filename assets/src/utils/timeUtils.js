...
// 两个时间相隔多少天,date2 > date1
export function getSubDay (date1, date2) {
    if (!typeUtils.isDate(date1) || !typeUtils.isDate(date2)) {
        date1 = new Date(date1)
        date2 = new Date(date2)
    }
    return Math.abs(date2.getTime() - date1.getTime()) / (1000 * 3600 * 24) + 1
}

// getMonthFirstDayTime 获取前面N个月的第一天 2011-11-11
export function getMonthFirstDayTime (beforeMonthNum) {
    let t = Date.now() - 86400000 * 30 * beforeMonthNum
    let d = new Date()
    d.setTime(t)
    d.setDate(1)
